...
# info
info__ <- c("nRa_chapter4", "C:/Users/L03054557/OneDrive/Edmundo-ITESM/2.Cursos Impartidos/Economic Dynamics and General Equilibrium Models/Models/4. Non-Ricardian Agents/nRa_chapter4.gcn", "2019-10-21 15:06:34", "false")
...
vareqmap__ <- sparseMatrix(i = c(1, 1, 2, 2, 2, 2, 3, 4, 4, 4,
                                 ...
                                 17, 17, 17, 18, 18, 18),
                           j = c(1, 14, 2, 3, 10, 13, 3, 6, 12, 17,
                                 7, 8, 9, 10, 3, 10, 13, 17, 3, 10,
                                 13, 18, 2, 5, 5, 11, 17, 6, 12, 17,
                                 4, 5, 6, 8, 9, 11, 12, 13, 1, 2,
                                 10, 13, 17, 18, 2, 5, 8, 9, 11, 17,
                                 5, 11, 15, 6, 12, 16),
...
ss_eq__ <- function(v, pc, pf)
{
    r <- numeric(18)
    r[1] = -v[1] + v[14]
    r[2] = -v[2] + pf[1] * v[3] * v[10]^(-1 + pf[1]) * v[13]^(1 - pf[1])
    r[3] = -v[3] + exp(pf[6] * log(v[3]))
    r[4] = -v[6] + v[12] * v[17]
    r[5] = -v[7] + pf[5] * v[8]
    r[6] = -v[10] + pf[5] * v[9]
    r[7] = -v[17] + v[3] * (1 - pf[1]) * v[10]^pf[1] * v[13]^(-pf[1])
    r[8] = -v[18] + v[3] * v[10]^pf[1] * v[13]^(1 - pf[1])
    r[9] = pf[2] * (pf[4] * v[2] * v[5]^-1 + pf[4] * v[5]^-1 * (1 - pf[3])) - pf[4] * v[5]^-1
    r[10] = -(1 - pf[4]) * (1 - v[11])^-1 + pf[4] * v[5]^-1 * v[17]
    r[11] = -(1 - pf[4]) * (1 - v[12])^-1 + pf[4] * v[6]^-1 * v[17]
    r[12] = -v[4] + pf[5] * v[5] + v[6] * (1 - pf[5])
    r[13] = v[8] - v[9] + v[9] * (1 - pf[3])
    r[14] = -v[13] + pf[5] * v[11] + v[12] * (1 - pf[5])
    r[15] = -v[1] + v[18] - v[2] * v[10] - v[13] * v[17]
    r[16] = -v[5] - v[8] + v[2] * v[9] + v[11] * v[17]
    r[17] = v[15] - pf[2] * v[15] - pf[4] * log(v[5]) - log(1 - v[11]) * (1 - pf[4])
    r[18] = v[16] - pf[2] * v[16] - pf[4] * log(v[6]) - log(1 - v[12]) * (1 - pf[4])
...
ss_calibr_eq_jacob__ <- function(v, pc, pf)
{
    ...
    jac[2] = 1
    jac[3] = -1
    jac[4] = pf[1] * v[10]^(-1 + pf[1]) * v[13]^(1 - pf[1])
    jac[5] = pf[1] * v[3] * (-1 + pf[1]) * v[10]^(-2 + pf[1]) * v[13]^(1 - pf[1])
    jac[6] = pf[1] * v[3] * (1 - pf[1]) * v[10]^(-1 + pf[1]) * v[13]^(-pf[1])
    ...
    jac[13] = pf[5]
    jac[14] = -1
    jac[15] = (1 - pf[1]) * v[10]^pf[1] * v[13]^(-pf[1])
    jac[16] = pf[1] * v[3] * (1 - pf[1]) * v[10]^(-1 + pf[1]) * v[13]^(-pf[1])
    jac[17] = -pf[1] * v[3] * (1 - pf[1]) * v[10]^pf[1] * v[13]^(-1 - pf[1])
    jac[18] = -1
    jac[19] = v[10]^pf[1] * v[13]^(1 - pf[1])
    jac[20] = pf[1] * v[3] * v[10]^(-1 + pf[1]) * v[13]^(1 - pf[1])
    jac[21] = v[3] * (1 - pf[1]) * v[10]^pf[1] * v[13]^(-pf[1])
    jac[22] = -1
    jac[23] = pf[2] * pf[4] * v[5]^-1
    jac[24] = pf[2] * (-pf[4] * v[2] * v[5]^-2 - pf[4] * v[5]^-2 * (1 - pf[3])) + pf[4] * v[5]^-2
    jac[25] = -pf[4] * v[5]^-2 * v[17]
    jac[26] = -(1 - pf[4]) * (1 - v[11])^-2
    jac[27] = pf[4] * v[5]^-1
    jac[28] = -pf[4] * v[6]^-2 * v[17]
    jac[29] = -(1 - pf[4]) * (1 - v[12])^-2
    jac[30] = pf[4] * v[6]^-1
    jac[31] = -1
    jac[32] = pf[5]
    jac[33] = 1 - pf[5]
    ...
    jac[50] = v[11]
    jac[51] = -pf[4] * v[5]^-1
    jac[52] = (1 - pf[4]) * (1 - v[11])^-1
    jac[53] = 1 - pf[2]
    jac[54] = -pf[4] * v[6]^-1
    jac[55] = (1 - pf[4]) * (1 - v[12])^-1
    jac[56] = 1 - pf[2]
...
{
    Atm1x <- numeric(7)
    Atm1x[1] = pf[1] * v[3] * (-1 + pf[1]) * v[10]^(-2 + pf[1]) * v[13]^(1 - pf[1])
    Atm1x[2] = pf[6] * v[3]^-1 * exp(pf[6] * log(v[3]))
    Atm1x[3] = pf[1] * v[3] * (1 - pf[1]) * v[10]^(-1 + pf[1]) * v[13]^(-pf[1])
    Atm1x[4] = pf[1] * v[3] * v[10]^(-1 + pf[1]) * v[13]^(1 - pf[1])
    Atm1x[5] = 1 - pf[3]
    Atm1x[6] = -v[2]
    Atm1x[7] = v[2]
    Atm1 <- sparseMatrix(i = c(2, 3, 7, 8, 13, 15, 16),
                         j = c(10, 3, 10, 10, 9, 10, 9),
    ...
    Atx[2] = 1
    Atx[3] = -1
    Atx[4] = pf[1] * v[10]^(-1 + pf[1]) * v[13]^(1 - pf[1])
    Atx[5] = pf[1] * v[3] * (1 - pf[1]) * v[10]^(-1 + pf[1]) * v[13]^(-pf[1])
    Atx[6] = -1
    Atx[7] = -1
    Atx[8] = v[17]
    Atx[9] = v[12]
    ...
    Atx[12] = pf[5]
    Atx[13] = -1
    Atx[14] = (1 - pf[1]) * v[10]^pf[1] * v[13]^(-pf[1])
    Atx[15] = -pf[1] * v[3] * (1 - pf[1]) * v[10]^pf[1] * v[13]^(-1 - pf[1])
    Atx[16] = -1
    Atx[17] = v[10]^pf[1] * v[13]^(1 - pf[1])
    Atx[18] = v[3] * (1 - pf[1]) * v[10]^pf[1] * v[13]^(-pf[1])
    Atx[19] = -1
    Atx[20] = pf[4] * v[5]^-2
    Atx[21] = -pf[4] * v[5]^-2 * v[17]
    Atx[22] = (-1 + pf[4]) * (1 - v[11])^-2
    Atx[23] = pf[4] * v[5]^-1
    Atx[24] = -pf[4] * v[6]^-2 * v[17]
    Atx[25] = (-1 + pf[4]) * (1 - v[12])^-2
    Atx[26] = pf[4] * v[6]^-1
    Atx[27] = -1
    Atx[28] = pf[5]
    Atx[29] = 1 - pf[5]
    ...
    Atx[44] = v[11]
    Atx[45] = -pf[4] * v[5]^-1
    Atx[46] = (1 - pf[4]) * (1 - v[11])^-1
    Atx[47] = 1
    Atx[48] = -pf[4] * v[6]^-1
    Atx[49] = (1 - pf[4]) * (1 - v[12])^-1
    Atx[50] = 1
    At <- sparseMatrix(i = c(1, 1, 2, 2, 2, 3, 4, 4, 4, 5,
                       ...
                       j = c(1, 14, 2, 3, 13, 3, 6, 12, 17, 7,
                             8, 9, 10, 3, 13, 17, 3, 13, 18, 5,
                             5, 11, 17, 6, 12, 17, 4, 5, 6, 8,
                             9, 11, 12, 13, 1, 2, 13, 17, 18, 2,
                             5, 8, 11, 17, 5, 11, 15, 6, 12, 16),
                         x = Atx, dims = c(18, 18))
